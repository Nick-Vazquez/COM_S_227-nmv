...
import static mini2.State.isBoundary;
...
public class PearlUtil
{
  ...
  public static State[] createFromString(String text)
  {
    ...
    State[] states = new State[cleanText.length()];
    Scanner stateScanner = new Scanner(cleanText);

    for (int i = 0; i < cleanText.length(); i++)
    {
      char textState = stateScanner.findInLine(".").charAt(0);
      states[i] = getValue(textState);
    }
  ...
  {
    // TODO
    State[] allowableStates = {EMPTY, OPEN_GATE, PORTAL};

    boolean containsMovable = findRightmostMovableBlock(states, states.length - 1) != -1;
    boolean length = states.length >= 2;
    boolean correctState = false;
    boolean oneBoundary;

    int boundaries = 0;

    for (State state : allowableStates)
    {
      if (states[0] == state)
      {
        correctState = true;
    ...
    {
      // TODO: If this fails, change to false
      if (isBoundary(states[i], containsMovable) && findRightmostMovableBlock(states, i) == -1)
      {
        boundaries++;
      }
    }

    oneBoundary = boundaries == 1;

    return length && correctState && oneBoundary && isBoundary(states[states.length - 1], containsMovable);
  }
  ...
  public static int movePlayer(State[] states) 
  {
    ...
    collectPearls(states, 0, states.length - 1);
    ...
    if (containsMovable)
    {
      int leftMostMovable = states.length;
      ...
      return leftMostMovable - 1;
    }
    else if (states[states.length - 1] == SPIKES_ALL)
    {
  ...
  {
    // TODO
    // Remove all mergable blocks and replace with empty states
    for (int i = states.length - 2; i >= 0; i--)
    {
      int offset = 0;
      if (isMovable(states[i]))
      {
        while (states[i + 1 + offset] == EMPTY || states[i + 1 + offset] == PEARL)
        {
          states[i + 1 + offset] = states[i + offset];
          states[i + offset] = EMPTY;
          offset++;
        }
      }
    }
